    ...
    foreach ($attempts as $i => $attempt) {
        ...
        if ($format == 'lms') {
            if ($attempt->status == 'completed' || $attempt->status == 'incomplete') {
                ...
                $row[] = '<a href="'.$urlP3.'">'.get_string($attempt->status, 'scormlite').'</a>';            
            } else {
                $row[] = get_string('started', 'topaze');            
            }
        } else {
            if ($attempt->status == 'completed' || $attempt->status == 'incomplete') {
                $row[] = get_string($attempt->status.'csv', 'scormlite');            
            } else {
                $row[] = get_string('startedcsv', 'topaze');            
            }
        }
