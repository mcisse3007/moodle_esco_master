...
$url = new moodle_url('/mod/scormlite/report/report.php', array('id'=>$id));
$PAGE->set_url($url);
...
foreach ($records as $record) {
	if (!array_key_exists($record->userid, $students) && !is_guest($context, $record->userid)) {
		...
		$student->score = null;
		$student->trainer = has_capability('mod/scormlite:viewotherreport', get_context_instance(CONTEXT_MODULE, $cm->id), $student->id);
		// Score statistics
...
if (empty($students)) {
	...
	$table->start_output();
	$reviewallowed = has_capability('mod/scormlite:reviewothercontent', get_context_instance(CONTEXT_MODULE, $cm->id)) && $scoclosed;
	foreach ($students as $userid => $student) {
		$row = array();	
		// Check
		...
		//$strstatus = '<img src="'.$OUTPUT->pix_url($student->status, 'scormlite').'" alt="'.$strstatus.'" title="'.$strstatus.'" />';
		$row[] = $strstatus;
		// Score
		$review_link = null;
	...
	$table->finish_output();
	if ($scoclosed) {
		echo '<input type="submit" value="'.get_string("deleteattemps", "scormlite").'" onClick="return confirmSubmit()"/>';
	} else {
		echo '<button type="button" onClick="alert(\''.get_string("deleteattempsno", "scormlite").'\')">'.get_string("deleteattemps", "scormlite").'</button>';		
	}
	echo '</form>';
}
