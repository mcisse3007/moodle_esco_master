...
function scormlite_add_instance($data, $mform=null) {
    ...
    scormlite_grade_item_update($data);
...
function scormlite_update_instance($data, $mform) {
	...
	$transaction = $DB->start_delegated_transaction();
	{
		scormlite_save_sco($data, $mform, $data->coursemodule, 'packagefile');
		$data->timemodified = time();
...
function scormlite_grade_item_update($activity, $grades=null) {
    ...
    return grade_update('mod/scormlite', $activity->course, 'mod', 'scormlite', $activity->id, 0, $grades, $params);
}
