...
function richmedia_add_instance($richmedia, $mform = null) {
    ...
    if (is_int($richmedia->referencessubtitles)) {
    ...
    if ($mform) {
        ...
        if ($filenamesubtitles !== false) {
            $mform->save_stored_file('referencessubtitles', $context->id, 'mod_richmedia', 'subtitles', 0, '/', $filenamesubtitles);
            $record->referencessubtitles = $filenamesubtitles;
        }
    }
...
function richmedia_update_instance($richmedia, $mform = null) {
    ...
    if (is_numeric($richmedia->referenceslides)) {
        $richmedia->referenceslides = '';
    }
    if (is_numeric($richmedia->referencesxml)) {
        $richmedia->referencesxml = '';
    }
    if (is_numeric($richmedia->referencesvideo)) {
        $richmedia->referencesvideo = '';
    }
    if (is_numeric($richmedia->referencesfond)) {
        $richmedia->referencesfond = '';
    }
    if (is_numeric($richmedia->referencessubtitles)) {
        $richmedia->referencessubtitles = '';
    }
    ...
    if ($mform) {
        ...
        $filenamesubtitles = $mform->get_new_filename('referencessubtitles');
        if ($filenamesubtitles !== false) {
            $richmedia->referencessubtitles = $filenamesubtitles;
            $fs->delete_area_files($context->id, 'mod_richmedia', 'subtitles');
            $mform->save_stored_file('referencessubtitles', $context->id, 'mod_richmedia', 'subtitles', 0, '/', $filenamesubtitles);
        }
    }
