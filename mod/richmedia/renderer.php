...
class mod_richmedia_renderer extends plugin_renderer_base {
            ...
            if ($keywords) {
                if (richmedia_webtv_exists()) {
                    foreach ($keywords as $keyword) {
                        $output .= '<a href="' . $CFG->wwwroot . '/blocks/richmedia_catalog/results.php?search=' . $keyword . '">' . $keyword . '</a>&nbsp;';
                    }
                } else {
                    foreach ($keywords as $keyword) {
                        $output .= $keyword . ' ';
    ...
    public function search_form($search = '',$order = 'added',$direction = 'desc'){
        $url = new moodle_url('/blocks/richmedia_catalog/results.php', array('search'=>$search));
        $orderOptions = array('presentor' => 'Auteur', 'added' => 'Date d\'ajout',  'name' => 'Titre');
        $directions = array('desc' => 'Descendant', 'asc' => 'Ascendant');

        $output = '<form action="'.$url.'" method="GET" id="filter-form">';
        $output .= '<div class="search">';
        $output .= '<input type="text" name="search" value="'.$search.'" />&nbsp;&nbsp;&nbsp;';
